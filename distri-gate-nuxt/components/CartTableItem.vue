...
<td>
    <div class="flex items-center gap-3">
        ...
        <div>
        <div class="font-bold">{{ item.productTitle }}</div>
        <div class="text-sm opacity-50">{{ item.productVariant.name }}</div>
        </div>
    </div>
</td>
...
<td>
    ...
    <div class="flex gap-2">
        <button class="btn btn-ghost btn-sm text-xl" @click="cartstore.changeQuantity(item.variantId,'decrease')">-</button>
        <span class="font-bold w-3 text-center flex items-center justify-center">
            {{ item.quantity }}
        </span>
        <button class="btn btn-ghost btn-sm text-xl" @click="cartstore.changeQuantity(item.variantId,'increase')">+</button>

    </div>
    
</td>

<td class="w-[100px]">{{cartstore.currency.symbol}} {{ (item.productVariant.price_amount*item.quantity).toFixed(2) }}</td>

<th>
    <div class="tooltip font-light hover:tooltip-open tooltip-left" data-tip="Remove item">
        <button class="btn btn-ghost btn-sm" @click="cartstore.removeItem(item.variantId)">
